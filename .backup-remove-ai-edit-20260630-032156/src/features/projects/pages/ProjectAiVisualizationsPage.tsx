      ...
      <div className="w-full max-w-3xl overflow-hidden rounded-[1.4rem] bg-white shadow-2xl">
        <form onSubmit={handleSubmit}>
          <div className="flex items-center justify-between gap-3 border-b border-slate-100 px-4 py-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-xs font-bold text-slate-500">{mode === 'edit' ? 'اكتب التعديل وأضف صورة مرجعية.' : 'اختر صورة وأضف مراجع التصميم.'}</p>
              </div>
            </div>
          ...
          <div className="max-h-[72vh] space-y-4 overflow-y-auto px-4 py-4">
            ...
            {editTarget ? (
              <div className="rounded-2xl border border-[#50683f]/15 bg-[#50683f]/5 p-3">
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    <p className="text-xs font-black text-slate-800">التعديل سيتم على هذا التصميم.</p>
                    <p className="mt-1 text-[11px] font-bold text-slate-500">أضف صورة مرجعية واحدة على الأقل، مثل بلاط أو دهان.</p>
                  </div>
                </div>
              </div>
            ) : null}
            ...
            {formError || errorMessage ? <div className="rounded-2xl bg-rose-50 p-3 text-xs font-bold text-rose-600">{formError ?? errorMessage}</div> : null}
          </div>
...
function ReferenceImagesStrip({ previews, onRemove, onClear }: { previews: FilePreview[]; onRemove: (index: number) => void; onClear: () => void }) {
  if (previews.length === 0) {
    return <div className="flex min-h-[98px] items-center justify-center rounded-2xl border border-slate-200 bg-white px-4 py-4 text-center text-xs font-bold text-slate-400">أضف صورة مرجعية واحدة على الأقل</div>
  }
...
function getAiVisualizationErrorMessage(error: unknown) {
  const message = error instanceof Error && error.message === 'REFERENCE_IMAGE_REQUIRED'
    ? 'أضف صورة مرجعية واحدة على الأقل.'
