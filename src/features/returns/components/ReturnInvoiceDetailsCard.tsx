...
interface ReturnInvoiceDetailsCardProps {
  workItems: WorkItem[]
  workItemId: string
  supplierName: string
  notes: string
...
export function ReturnInvoiceDetailsCard({
  workItems,
  workItemId,
  supplierName,
  notes,
  ...
  return (
    <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-[0_18px_44px_rgb(var(--color-brand-ink-rgb)/0.07)]">
      <div className="mb-6 flex items-center gap-3 border-b border-slate-100 pb-5">
        <span className="flex h-11 w-11 items-center justify-center rounded-2xl bg-orange-50 text-orange-600"><InvoiceIcon name="archive" /></span>
        <div>
          <h2 className="text-xl font-black text-slate-950">بيانات المسترجع</h2>
          <p className="mt-1 text-sm font-semibold text-slate-500">اختر البند والمورد ثم أضف المواد والكميات المسترجعة.</p>
        </div>
      </div>
      ...
      <div className="grid gap-5 md:grid-cols-2">
        ...
        <label className="space-y-2 text-right md:col-span-2">
          ...
          <textarea
            ...
            placeholder="سبب الاسترجاع أو أي ملاحظات إضافية..."
            rows={3}
