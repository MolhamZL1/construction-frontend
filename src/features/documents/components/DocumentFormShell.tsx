  ...
  return (
    <section className="min-h-[calc(100vh-4rem)] bg-white px-5 py-7 sm:px-8 lg:px-10" dir="rtl">
      <div className="mx-auto max-w-4xl space-y-6">
        <nav className="flex flex-wrap items-center justify-end gap-2 text-sm font-bold text-slate-500">
          {breadcrumbs.map((item, index) => (
            <span key={`${item.label}-${index}`} className="inline-flex items-center gap-2">
              ...
              {item.to ? (
                <Link to={item.to} className="transition hover:text-[#50683f]">
                  {item.label}
                </Link>
              ) : (
        ...
        <div className="rounded-3xl border border-slate-200 bg-white p-6 text-right shadow-[0_14px_40px_rgba(15,23,42,0.07)] md:p-8">
