...
import { getProjectsErrorMessage, useCreateSpace, useProjectSummary } from '../hooks/useProjects'

export function CreateProjectSpacePage() {
  ...
  return (
    <section className="min-h-[calc(100vh-4rem)] bg-white px-5 py-7 sm:px-8 lg:px-10" dir="rtl">
      <div className="mx-auto max-w-4xl space-y-6">
        ...
        <header className="rounded-3xl border border-slate-200 bg-white p-5 text-right shadow-[0_12px_32px_rgba(15,23,42,0.07)] sm:p-6 md:p-7">
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <h1 className="mt-1 text-2xl font-black text-slate-900 sm:text-3xl">إضافة فراغ جديد</h1>
              <p className="mt-2 text-sm font-semibold leading-6 text-slate-500">
                حدد نوع الفراغ ومساحات الجدران والسقف والتشطيبات المطلوبة.
              </p>
            </div>
          </div>
        </header>
        ...
        <SpaceForm
          submitLabel="حفظ الفراغ"
          isSubmitting={createMutation.isPending}
          errorMessage={createMutation.error ? getProjectsErrorMessage(createMutation.error) : null}
