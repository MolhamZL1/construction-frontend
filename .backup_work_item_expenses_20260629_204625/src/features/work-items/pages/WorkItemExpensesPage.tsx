...
import { useAddWorkItemExpense, useWorkItemExpenses } from '../hooks/useWorkItemExpenses'
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <p className="mt-2 text-sm font-medium text-slate-500">
              تسجيل ومتابعة مصاريف كل بند عمل ضمن المشروع{project ? `: ${project.name}` : ''}
            </p>
          </div>
          ...
          <>
            ...
            <section className="grid gap-5 lg:grid-cols-[380px_minmax(0,1fr)]">
              <form onSubmit={handleSubmit} className="space-y-5 rounded-3xl border border-slate-200 bg-white p-5 shadow-[0_14px_40px_rgba(15,23,42,0.07)]">
                <div>
                  <h2 className="text-xl font-extrabold text-slate-900">إضافة مصروف</h2>
                  <p className="mt-1 text-sm font-semibold text-slate-500">سجل قيمة المصروف ووصفه للبند المحدد.</p>
                </div>
                ...
                >
                  {addExpenseMutation.isPending ? 'جاري إضافة المصروف...' : 'إضافة المصروف'}
                </button>
              </form>
...
function ExpensesList({ isLoading, expenses }: { isLoading: boolean; expenses: Array<{ id: string; amountLabel: string; description: string; createdBy?: { name: string }; createdAt?: string | null }> }) {
  if (isLoading) {
  ...
  if (expenses.length === 0) {
    return (
      <div className="flex min-h-[360px] flex-col items-center justify-center rounded-3xl border border-dashed border-slate-200 bg-slate-50 px-6 text-center">
        ...
        <p className="mt-2 max-w-sm text-sm font-semibold leading-6 text-slate-500">أضف أول مصروف للورشة من النموذج الموجود بجانب القائمة.</p>
      </div>
    )
  }
