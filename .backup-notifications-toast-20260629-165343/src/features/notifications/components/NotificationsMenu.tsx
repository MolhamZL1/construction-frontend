...
import { getBrowserNotificationStatus, type BrowserNotificationStatus } from '@/lib/firebase'
...
function notificationStatusText(status: BrowserNotificationStatus) {
  if (status === 'granted') return 'إشعارات المتصفح مفعلة'
  if (status === 'denied') return 'الإشعارات موقوفة من إعدادات المتصفح'
  if (status === 'unsupported') return 'المتصفح لا يدعم إشعارات الويب'
  if (status === 'missing-vapid-key') return 'إشعارات النظام تعمل، وإشعارات المتصفح تحتاج تفعيل المفتاح'
  return 'فعّل إشعارات المتصفح لتصلك التنبيهات فوراً'
}
...
export function NotificationsMenu() {
  ...
  const canEnableBrowserNotifications = browserStatus === 'default'
  ...
  async function handleEnableNotifications() {
    ...
    try {
      const result = await registerAuthenticatedDevice({ requestPermission: true, forceSend: true })
      setBrowserStatus(getBrowserNotificationStatus())
      setFeedback(result.ok ? 'تم تفعيل إشعارات المتصفح بنجاح.' : 'لم يتم التفعيل. تأكد من السماح للإشعارات من المتصفح.')
    } catch {
      setFeedback('لم يتم التفعيل. جرّب مرة ثانية لاحقاً.')
    } finally {
      setIsEnabling(false)
    }
  }
          ...
          <div className="border-b border-slate-100 px-4 py-4">
            ...
            <div className="mt-4 rounded-2xl bg-slate-50 px-3 py-3">
              <div className="flex items-center justify-between gap-3">
                <p className="text-xs leading-5 text-slate-600">{notificationStatusText(browserStatus)}</p>
                {canEnableBrowserNotifications ? (
                  <button
                    type="button"
                    onClick={handleEnableNotifications}
                    disabled={isEnabling}
                    className="shrink-0 rounded-xl bg-[#50683f] px-3 py-2 text-xs font-semibold text-white transition hover:bg-[#425734] disabled:opacity-60"
                  >
